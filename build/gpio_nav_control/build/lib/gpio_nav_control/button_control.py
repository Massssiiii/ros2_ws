...
class ButtonControlNode(Node):
    def __init__(self):
        ...
        self.sequence_timeout = 100.0  # Temps maximum en secondes

        self.obstacle_detection_enabled = True  # Variable pour activer/désactiver la détection
    ...
    def goal_switch_released(self):
        ...
        # Démarrer le timer global de 100 secondes dès maintenant
        self.sequence_start_time = time.time()
        self.get_logger().info(f"Démarrage du timer global ({self.sequence_timeout}s)")
    ...
    def lidar_callback(self, msg):
        ...
        # Sortir immédiatement si la détection est désactivée
        if not self.obstacle_detection_enabled:
            self.obstacle_detected = False
            return
            
        # Continue seulement si le mode est manuel et la détection activée
        if self.mode == "manu":
            ...
            if distances and min(distances) < 0.18:
                ...
                if not self.obstacle_detected:
                    ...
                    self.get_logger().warn(f"Obstacle détecté à moins de {min_distance:.2f}m ! Arrêt du robot.")
                    # Sauvegarder l'action en cours avant de l'arrêter
                    if self.current_action and self.manual_sequence_active:
                        self.paused_action = self.current_action.copy()
                        self.paused_action['elapsed_time'] = time.time() - self.action_start_time
                        self.get_logger().info("Action en cours sauvegardée pour reprise ultérieure")
                    self.stop_robot()
            elif self.obstacle_detected:
                # L'obstacle n'est plus détecté
                self.obstacle_detected = False
                self.get_logger().info("Obstacle disparu, reprise du mouvement possible")
                
                # Reprendre l'action qui était en cours si on était dans une séquence
                if hasattr(self, 'paused_action') and self.paused_action and self.manual_sequence_active:
                    self.get_logger().info(f"Reprise de l'action: {self.paused_action['type']}")
                    
                    # Calculer le temps restant pour l'action
                    remaining_duration = self.paused_action['duration'] - self.paused_action['elapsed_time']
                    
                    if remaining_duration > 0:
                        # Redémarrer le mouvement avec le temps restant
                        self.start_movement(
                            linear=self.paused_action['linear'],
                            angular=self.paused_action['angular'],
                            duration=remaining_duration
                        )
                        
                    # Nettoyer après reprise
                    self.paused_action = None
            
    def disable_obstacle_detection(self):
        """Désactive la détection d'obstacles et réinitialise l'état d'obstacle"""
        self.obstacle_detection_enabled = False
        self.obstacle_detected = False  # Réinitialiser l'état d'obstacle
        self.get_logger().info("Détection d'obstacles désactivée")
        return True

    def enable_obstacle_detection(self):
        """Active la détection d'obstacles"""
        self.obstacle_detection_enabled = True
        self.get_logger().info("Détection d'obstacles activée")
        return True        
    ...
    def timer_callback(self):
        ...
        if self.current_action:
            if time.time() - self.action_start_time >= self.current_action['duration']:
                ...
                # On ne fait qu'arrêter le robot, mais on ne passe pas à l'étape suivante
                # car on veut reprendre cette action une fois l'obstacle disparu
                self.stop_robot()
                self.get_logger().warn("Action mise en pause par un obstacle")
                # Ne pas effacer current_action pour pouvoir s'en souvenir quand l'obstacle disparaîtra

    def wait(self, duration):
        """Attend pendant une durée spécifiée avant de passer à l'étape suivante"""
        self.current_action = {
            'type': 'wait',
            'duration': duration,
            'linear': 0.0,
        ...
        self.get_logger().info(f"Attente de {duration} secondes...")
        return True           
    ...
    def start_movement(self, linear=0.0, angular=0.0, duration=1.0):
        ...
        if self.obstacle_detected and self.obstacle_detection_enabled and linear > 0:
    ...
    def move_backward(self, distance):
        ...
        speed = (-0.15)  # m/s
        duration = distance / speed
        return self.start_movement(linear=speed, duration=2)
    ...
    def start_manual_sequence(self, color):
        ...
        # Ne pas réinitialiser sequence_start_time car il a été défini dans goal_switch_released
    ...
    def stop_manual_sequence(self):
        ...
        self.current_action = None
        self.manual_sequence_step = 0
        self.enable_obstacle_detection()  # S'assurer que la détection est activée par défaut
        self.stop_robot()
...
#                lambda: self.finish_manual_sequence()
                ...
                lambda: self.turn_left(83),
                lambda: self.move_forward(0.45),
                lambda: self.turn_left(80),
                lambda: self.disable_obstacle_detection() and self.move_forward(0.35),
                #lambda: self.disable_obstacle_detection(),  # Désactiver la détection après la deuxième rotation
                #lambda: self.move_forward(0.50),
                lambda: self.move_backward(1.0),
                lambda: self.turn_right(181) and self.enable_obstacle_detection(),
                lambda: self.wait(70.0),  # Ajoute un délai de 2 secondes
                lambda: self.move_forward(0.9),
                
                #lambda: self.enable_obstacle_detection(),   # Réactiver la détection à la fin (optionnel)
                lambda: self.finish_manual_sequence()
            ]
...
#                lambda: self.finish_manual_sequence()
                lambda: self.move_forward(0.4),
                lambda: self.turn_right(83),
                lambda: self.move_forward(0.45),
                lambda: self.turn_right(80),
                lambda: self.disable_obstacle_detection() and self.move_forward(0.35),
                #lambda: self.disable_obstacle_detection(),  # Désactiver la détection après la deuxième rotation
                #lambda: self.move_forward(0.50),
                lambda: self.move_backward(1.0),
                lambda: self.turn_left(181) and self.enable_obstacle_detection(),
                lambda: self.wait(70.0),  # Ajoute un délai de 2 secondes
                lambda: self.move_forward(0.9),
